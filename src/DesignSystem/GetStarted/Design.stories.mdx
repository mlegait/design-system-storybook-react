...
import grid2 from '../images/grid-2.gif';
import carbonKit from '../images/carbon-kit.png';
import textStyles1 from '../images/text-styles-1.png';
import layerStyle1 from '../images/layer-style-1.png';
...
There are [four Carbon themes](internalLink), two light (White and Gray 10) and two dark (Gray 90 and Gray 100). Each theme lives in its own Sketch library. You can subscribe to as many libraries as you’d like.

TODO external block link component
...
TODO external link block component
...
<img src={grid2} alt="2x Grid overview" />

All designs should start with the 2x Grid template. Once you have [saved the grid](internalLink) as a template, navigate to `File → New file from Template` and select the grid template.

See our [layout guidelines](internalLink) for a more detailed walkthrough of the 2x Grid.
...
Carbon [components](internalLink), [add-ons](internalLink), and [icons](internalLink) live in the design kit as Sketch symbols.

<img src={carbonKit} alt="Carbon symbol menu navigation" />
...
- For even more configurablility, use the individual building blocks found under the `Items` folder within the symbol. Be sure your custom design adheres to our [design guidelines](internalLink).
...
- The first text style level displays [theme options](internalLink) (i.e., white theme).
- The second level shows the [Carbon type](internalLink) tokens (i.e., body-long-01).
- The third level selects the type color, which is labeled with a [Carbon color](internalLink) token name.

## Layer styles

Layer styles are color selections for any shape in Sketch.

<img src={layerStyle1} alt="Layer styles navigation example" />

### Library menu navigation

[Color](internalLink) in Carbon is carefully considered, and we discourage custom layer styles.
...
If you’re using an older version of Carbon, check out the v10 [migration docs](internalLink) when you’re ready to make the switch.
